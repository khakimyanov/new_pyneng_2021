...
def send_config_command_set(device, command_set):
    ...
    with ConnectHandler(**device) as ssh:
        ssh.enable()
        result = ssh.send_config_set(command_set, strip_command=False)
...
def send_show_command(device, show):
    ...
    with ConnectHandler(**device) as ssh:
        ...
        result = ssh.send_command(show, strip_command=False)
        logging.info(received_msg.format(datetime.now().time(), ip))

    return prompt + result + '\n'
    #Как всегда Наталья это делает более элегантно
    #return f"{prompt}{command}\n{result}\n"

def send_commands_to_devices(devices, filename, *, show=None, config=None, limit=3):
    if show and config:
        raise ValueError('Function takes only 2 positional argument, 3 were given')
    with ThreadPoolExecutor(max_workers=limit) as executor:
        #Вместо такого моего if у Натальи
        #command = show if show else config
        #function = send_show_command if show else send_config_command_set
        #и тогда запуск
        #futures = [executor.submit(function, device, command) for device in devices]

        if show:
            f_result = [executor.submit(send_show_command, device, show) for device in devices]
        elif config:
            f_result = [executor.submit(send_config_command_set, device, config) for device in devices]
...
if __name__ == "__main__":
    ...
    #send_commands_to_devices(devices, 'result.txt', show='sh clock', config='logging 1.1.1.1')
